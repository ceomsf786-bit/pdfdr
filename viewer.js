...
const els = {
  ...
  boardPanel:$('boardPanel'), toggleBoard:$('toggleBoard'), boardText:$('boardText'), boardLabel:$('boardLabel'), boardTitleInput:$('boardTitleInput'), boardTitleDisplay:$('boardTitleDisplay'),
  boardPrev:$('boardPrev'), boardNext:$('boardNext'), boardAdd:$('boardAdd'), boardDelete:$('boardDelete'),
  imageSection:$('imageSection'), imageStage:$('imageStage'), boardImage:$('boardImage'), imageCounter:$('imageCounter'), imagePrev:$('imagePrev'), imageNext:$('imageNext'), pasteImageBtn:$('pasteImageBtn'), deleteImageBtn:$('deleteImageBtn'), imageFileInput:$('imageFileInput'),
  ...
  toolSplit:$('toolSplit'), noteSplit:$('noteSplit'), noteInnerSplit:$('noteInnerSplit'), studentSplit:$('studentSplit'),
  loginPanel:$('loginPanel'), loginForm:$('loginForm'), emailInput:$('emailInput'), passwordInput:$('passwordInput'), signOutBtn:$('signOutBtn'),
  libraryBtn:$('libraryBtn'), libraryDialog:$('libraryDialog'), libraryList:$('libraryList'), createDocBtn:$('createDocBtn'), newTitle:$('newTitle'), newDriveUrl:$('newDriveUrl'), copyStudentLink:$('copyStudentLink'), exportPdf:$('exportPdf'),
...
const state = {
  ...
  boardOpen:true, teacherOnline:false, followTeacher:true, liveImageId:null, liveScrollRatio:0, scrollSyncTimer:null
};
...
function persistStudentState(){
  ...
  try{ localStorage.setItem(`sntpdf:${state.doc.id}`,JSON.stringify({pageNo:state.pageNo,boardNo:state.boardNo,scale:state.scale,fit:state.fit,boardOpen:state.boardOpen,imageIndex:state.imageIndex})); }catch{}
}
...
async function loadTeacherBoards(){
  const {data,error}=await supabase.from('snt_pdf_boards').select('board_no,title,text_content,updated_at').eq('document_id',state.doc.id).order('board_no');if(error)throw error;
  state.boards=data||[];
  if(!state.boards.length){const {error:e}=await supabase.from('snt_pdf_boards').insert({document_id:state.doc.id,board_no:1,title:'General notes',text_content:'',objects:[]});if(e)throw e;state.boards=[{board_no:1,title:'General notes',text_content:''}];}
  if(!boardByNo())state.boardNo=state.boards[0].board_no;loadBoardText();await loadImages();
}
async function loadStudentBoards(){const r=await api('boards');state.boards=r.boards||[];if(!state.boards.length)state.boards=[{board_no:1,title:'Teacher notes',text_content:''}];if(!boardByNo())state.boardNo=state.boards[0].board_no;loadBoardText();await loadImages();}
function loadBoardText(){
  const b=boardByNo()||{board_no:state.boardNo,title:`Board ${state.boardNo}`,text_content:''};
  if(els.boardText)els.boardText.value=b.text_content||'';if(els.boardTitleInput)els.boardTitleInput.value=b.title||`Board ${state.boardNo}`;if(els.boardTitleDisplay)els.boardTitleDisplay.textContent=b.title||'Teacher Notepad';if(els.boardLabel)els.boardLabel.textContent=`Board ${b.board_no}`;
  const arr=state.boards.map(x=>Number(x.board_no)).sort((a,b)=>a-b),i=arr.indexOf(Number(state.boardNo));if(els.boardPrev)els.boardPrev.disabled=i<=0;if(els.boardNext)els.boardNext.disabled=i<0||i>=arr.length-1;
}
async function saveBoardNow(){
  if(!TEACHER||!state.doc)return;const b=boardByNo();if(!b)return;const title=(els.boardTitleInput?.value||b.title||`Board ${state.boardNo}`).trim().slice(0,80)||`Board ${state.boardNo}`,text=els.boardText?.value||'';
  const {error}=await supabase.from('snt_pdf_boards').upsert({document_id:state.doc.id,board_no:state.boardNo,title,text_content:text,objects:[],updated_at:new Date().toISOString()},{onConflict:'document_id,board_no'});if(error){setStatus(error.message);return;}
  b.title=title;b.text_content=text;setStatus('Notepad saved');
}
function saveBoardSoon(){clearTimeout(state.boardTextTimer);state.boardTextTimer=setTimeout(()=>saveBoardNow().catch(e=>setStatus(e.message)),220);}
async function addBoard(){
  const next=Math.max(0,...state.boards.map(x=>Number(x.board_no)))+1;const title=`Board ${next}`;const {error}=await supabase.from('snt_pdf_boards').insert({document_id:state.doc.id,board_no:next,title,text_content:'',objects:[]});if(error)return setStatus(error.message);state.boardNo=next;await loadTeacherBoards();await pushLiveState();
}
async function deleteBoard(){
  if(state.boards.length<=1)return setStatus('Keep at least one board.');const b=boardByNo();if(!confirm(`Delete “${b?.title||'this board'}” and its pasted images?`))return;
  for(const image of [...state.images]){
    const r=await fetch(apiUrl('delete-image',{image:image.id}),{method:'POST',headers:authHeaders()});
    if(!r.ok){const d=await r.json().catch(()=>({}));return setStatus(d.error||'Could not delete board image.');}
  }
  const {error}=await supabase.from('snt_pdf_boards').delete().eq('document_id',state.doc.id).eq('board_no',state.boardNo);if(error)return setStatus(error.message);
  state.boardNo=state.boards.find(x=>x.board_no!==b.board_no)?.board_no||1;state.liveImageId=null;await loadTeacherBoards();await pushLiveState();
}
async function moveBoard(delta){
  const arr=state.boards.map(x=>Number(x.board_no)).sort((a,b)=>a-b),i=arr.indexOf(Number(state.boardNo)),n=i+delta;if(n<0||n>=arr.length)return;state.boardNo=arr[n];loadBoardText();await loadImages();if(TEACHER)await pushLiveState();else persistStudentState();
}
...
async function showCurrentImage(){
  ...
  if(!count){els.boardImage?.classList.add('hidden');if(els.imageStage){const span=els.imageStage.querySelector('span');if(span)span.classList.remove('hidden');}state.liveImageId=null;return;}
  const image=state.images[state.imageIndex];try{const url=await fetchImageBlobUrl(image);els.boardImage.src=url;els.boardImage.alt=image.image_name||'Teacher pasted image';els.boardImage.classList.remove('hidden');const span=els.imageStage?.querySelector('span');if(span)span.classList.add('hidden');state.liveImageId=image.id;if(TEACHER)await pushLiveState();persistStudentState();}catch(e){setStatus(e.message);}
}
async function moveImage(delta){if(!state.images.length)return;const n=state.imageIndex+delta;if(n<0||n>=state.images.length)return;state.imageIndex=n;await showCurrentImage();}
...
async function pushLiveState(){
  if(!TEACHER||!state.doc)return;
  const maxScroll=Math.max(1,els.pdfScroller.scrollHeight-els.pdfScroller.clientHeight),scrollRatio=Math.max(0,Math.min(1,els.pdfScroller.scrollTop/maxScroll));state.liveScrollRatio=scrollRatio;
  const {error}=await supabase.from('snt_pdf_documents').update({live_page:state.pageNo,live_board_no:state.boardNo,live_image_id:state.liveImageId||null,live_scroll_ratio:scrollRatio,teacher_present_at:new Date().toISOString(),updated_at:new Date().toISOString()}).eq('id',state.doc.id);if(error)setStatus(error.message);
}
function teacherHeartbeat(){if(!TEACHER||!state.doc)return;pushLiveState().catch(()=>{});}
async function pollStudentSync(){
  if(TEACHER||!state.doc)return;
  try{
    const s=await api('sync');const wasOnline=state.teacherOnline,previousLiveImage=state.liveImageId,newRevision=Number(s.revision||0),revisionChanged=newRevision!==state.revision;state.teacherOnline=!!s.teacher_online;state.liveImageId=s.live_image_id||null;state.liveScrollRatio=Math.max(0,Math.min(1,Number(s.live_scroll_ratio||0)));
    if(els.liveText){els.liveText.textContent=state.teacherOnline?'Following teacher':'View mode';els.liveText.classList.toggle('following',state.teacherOnline);}
    if(els.prevPage)els.prevPage.disabled=state.teacherOnline||state.pageNo<=1;if(els.nextPage)els.nextPage.disabled=state.teacherOnline||state.pageNo>=state.pdf.numPages;if(els.pageInput)els.pageInput.disabled=state.teacherOnline;
    if(state.teacherOnline){
      const targetPage=Math.max(1,Number(s.live_page||1)),targetBoard=Math.max(1,Number(s.live_board_no||1));
      if(targetPage!==state.pageNo){await renderPage(targetPage,true);}else if(revisionChanged){await loadStudentPageData();await loadStudentBoards();}
      if(targetBoard!==state.boardNo){state.boardNo=targetBoard;loadBoardText();await loadImages();}
      else if((s.live_image_id||null)!==(previousLiveImage||null)){await loadImages();}
      const maxScroll=Math.max(0,els.pdfScroller.scrollHeight-els.pdfScroller.clientHeight);if(maxScroll>0){const target=Math.round(state.liveScrollRatio*maxScroll);if(Math.abs(els.pdfScroller.scrollTop-target)>8)els.pdfScroller.scrollTop=target;}
    }else if(revisionChanged){await loadStudentPageData();await loadStudentBoards();}
    state.revision=newRevision;
    if(!wasOnline&&state.teacherOnline)setStatus('Teacher is live — following their page');else if(wasOnline&&!state.teacherOnline)setStatus('Teacher offline — you can browse freely');
  }catch{}
}
...
async function renderPage(n,forceData=false){
  ...
  try{
    ...
    state.pageNo=n;if(pageChanged){els.pdfScroller.scrollTop=0;els.pdfScroller.scrollLeft=0;if(els.toolRail)els.toolRail.style.transform='translateY(0)';}els.pageInput.value=String(n);els.pageCount.textContent=`/ ${state.pdf.numPages}`;els.prevPage.disabled=(!TEACHER&&state.teacherOnline)||n<=1;els.nextPage.disabled=(!TEACHER&&state.teacherOnline)||n>=state.pdf.numPages;
    if(TEACHER){await loadTeacherPageData(forceData);await pushLiveState();window.requestIdleCallback?.(()=>prefetchAdjacent().catch(()=>{}));}
    else{await loadStudentPageData();persistStudentState();}
  }finally{state.rendering=false;if(state.pendingPage!==null){const p=state.pendingPage;state.pendingPage=null;renderPage(p);}}
}

function setBoardOpen(open){state.boardOpen=!!open;if(TEACHER){els.workspace.classList.toggle('notes-hidden',!state.boardOpen);els.boardPanel.style.display=state.boardOpen?'':'none';els.noteSplit.style.display=state.boardOpen?'':'none';if(state.fit&&state.pdf)setTimeout(()=>renderPage(state.pageNo),50);}else{els.workspace.classList.toggle('notes-hidden',!state.boardOpen);els.boardPanel.classList.toggle('closed',!state.boardOpen);}persistStudentState();}
...
function resetLayout(){document.documentElement.style.removeProperty('--tools-w');document.documentElement.style.removeProperty('--notes-w');document.documentElement.style.removeProperty('--note-text-h');document.documentElement.style.removeProperty('--student-note-h');try{localStorage.removeItem('sntpdf:teacher-layout');}catch{};state.fit=true;if(state.pdf)renderPage(state.pageNo);}
function bindLayout(){
  restoreLayout();
  ...
  bindResizer(els.studentSplit,e=>{const r=els.workspace.getBoundingClientRect(),h=Math.max(150,Math.min(r.height*.65,r.bottom-e.clientY));document.documentElement.style.setProperty('--student-note-h',`${h}px`);if(state.fit&&state.pdf)renderPage(state.pageNo);},()=>persistStudentState());
}
function bindCommon(){
  els.prevPage?.addEventListener('click',()=>{if(!TEACHER&&state.teacherOnline)return;renderPage(state.pageNo-1);});
  els.nextPage?.addEventListener('click',()=>{if(!TEACHER&&state.teacherOnline)return;renderPage(state.pageNo+1);});
  els.pageInput?.addEventListener('change',()=>{if(!TEACHER&&state.teacherOnline){els.pageInput.value=state.pageNo;return;}renderPage(els.pageInput.value);});
  els.zoomOut?.addEventListener('click',()=>{state.fit=false;state.scale=Math.max(.4,state.scale-.15);renderPage(state.pageNo);persistStudentState();});
  els.zoomIn?.addEventListener('click',()=>{state.fit=false;state.scale=Math.min(3.5,state.scale+.15);renderPage(state.pageNo);persistStudentState();});
  els.fitPage?.addEventListener('click',()=>{state.fit=true;renderPage(state.pageNo);persistStudentState();});
  els.toggleBoard?.addEventListener('click',()=>setBoardOpen(!state.boardOpen));
  els.boardPrev?.addEventListener('click',()=>{if(!TEACHER&&state.teacherOnline)return;moveBoard(-1);});els.boardNext?.addEventListener('click',()=>{if(!TEACHER&&state.teacherOnline)return;moveBoard(1);});
  els.imagePrev?.addEventListener('click',()=>{if(!TEACHER&&state.teacherOnline)return;moveImage(-1);});els.imageNext?.addEventListener('click',()=>{if(!TEACHER&&state.teacherOnline)return;moveImage(1);});
...
function bindTeacher(){
  ...
  els.pdfScroller?.addEventListener('scroll',()=>{
    ...
    clearTimeout(state.scrollSyncTimer);state.scrollSyncTimer=setTimeout(()=>pushLiveState().catch(()=>{}),160);
  },{passive:true});
...
async function createDocumentFromForm(){
  ...
  const row={owner_id:user.id,title,drive_file_id:fileId,drive_share_url:url,student_token:randomToken(),student_link_enabled:true,live_board_no:1};const {data,error}=await supabase.from('snt_pdf_documents').insert(row).select('*').single();if(error){setStatus(error.message);return;}await supabase.from('snt_pdf_boards').insert({document_id:data.id,board_no:1,title:'General notes',text_content:'',objects:[]});els.newTitle.value='';els.newDriveUrl.value='';els.libraryDialog.close();await openTeacherDocument(data);
}
async function openTeacherDocument(doc){
  state.doc=doc;state.pageNo=Math.max(1,Number(doc.live_page||1));state.boardNo=Math.max(1,Number(doc.live_board_no||1));state.liveImageId=doc.live_image_id||null;state.revision=Number(doc.revision||0);els.docTitle.textContent=doc.title;els.contextText.textContent='Teacher • fast local drawing • autosave';history.replaceState(null,'',`./teacher.html?id=${encodeURIComponent(doc.id)}`);await loadTeacherBoards();await loadPdf();els.loading.classList.add('hidden');els.workspace.classList.remove('hidden');setBoardOpen(true);teacherHeartbeat();clearInterval(state.heartbeatTimer);state.heartbeatTimer=setInterval(teacherHeartbeat,Number(CONFIG.TEACHER_HEARTBEAT_MS||5000));
}
...
async function bootStudent(){
  ...
  try{
    const init=await api('init');state.doc=init.document;state.revision=Number(init.document.revision||0);state.teacherOnline=!!init.teacher_online;state.liveImageId=init.document.live_image_id||null;state.liveScrollRatio=Math.max(0,Math.min(1,Number(init.document.live_scroll_ratio||0)));state.pageNo=Math.max(1,Number(init.document.live_page||1));state.boardNo=Math.max(1,Number(init.document.live_board_no||1));els.docTitle.textContent=init.document.title;els.contextText.textContent='Teacher annotations • view only';restoreStudentState();
    if(state.teacherOnline){state.pageNo=Math.max(1,Number(init.document.live_page||1));state.boardNo=Math.max(1,Number(init.document.live_board_no||1));}
    setBoardOpen(state.boardOpen!==false);await loadStudentBoards();await loadPdf();els.loading.classList.add('hidden');els.workspace.classList.remove('hidden');
