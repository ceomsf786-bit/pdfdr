// SNT PDF Annotator v3.17
// Direct, single-layer upgrade from viewer.js.
// Fixes selected existing/new shape opacity, outline width and colours without
// patching another loader. This avoids the v3.16 nested-patch fallback problem.
...
function replaceOnce(oldText, newText, label){
  ...
  if(pos < 0) throw new Error(`Viewer v3.17 upgrade mismatch: ${label}`);
...
replaceOnce(
  ...
  "toolRail:$('toolRail'), toolButtons:[...document.querySelectorAll('[data-tool]')], widthInput:$('widthInput'), colorInput:$('colorInput'), shapeFillToggle:$('shapeFillToggle'), shapeFillColor:$('shapeFillColor'), shapeFillOpacity:$('shapeFillOpacity'), shapeFillOpacityValue:$('shapeFillOpacityValue'), undoBtn:$('undoBtn'), redoBtn:$('redoBtn'), deleteSelected:$('deleteSelected'),",
  'shape controls'
);
...
if(drawStart < 0 || drawEnd < 0) throw new Error('Viewer v3.17 upgrade mismatch: drawing function');
src = src.slice(0,drawStart) + `function drawObject(ctx,o,canvas,selected=false){
  ...
  if(o.type==='pen'||o.type==='highlighter'){ const pts=o.points||[];if(pts.length){ctx.beginPath();pts.forEach((p,i)=>i?ctx.lineTo(X(p.x),Y(p.y)):ctx.moveTo(X(p.x),Y(p.y)));ctx.stroke();} }
    ...
    if(o.fill){ctx.save();ctx.globalAlpha=Math.max(0,Math.min(1,Number(o.fillOpacity??1)));ctx.fillStyle=o.fillColor||o.color||'#1f2937';ctx.fillRect(x,y,rw,rh);ctx.restore();}ctx.strokeRect(x,y,rw,rh);
  }
  else if(o.type==='ellipse'){
    ctx.beginPath();ctx.ellipse(X((o.x1+o.x2)/2),Y((o.y1+o.y2)/2),Math.abs(o.x2-o.x1)*w/2,Math.abs(o.y2-o.y1)*h/2,0,0,Math.PI*2);if(o.fill){ctx.save();ctx.globalAlpha=Math.max(0,Math.min(1,Number(o.fillOpacity??1)));ctx.fillStyle=o.fillColor||o.color||'#1f2937';ctx.fill();ctx.restore();}ctx.stroke();
  }
  if(selected){
...
// Keep the annotation rail visible while the PDF canvas itself scrolls.
replaceOnce(
...
`        else if(o.type==='rect'){const fillColor=hex(o.fillColor||o.color);page.drawRectangle({x:X(Math.min(o.x1,o.x2)),y:Y(Math.max(o.y1,o.y2)),width:Math.abs(o.x2-o.x1)*width,height:Math.abs(o.y2-o.y1)*height,borderColor:color,borderWidth:th,color:fillColor,opacity:o.fill?Math.max(0,Math.min(1,Number(o.fillOpacity??1))):0,borderOpacity:1});}
        else if(o.type==='ellipse'){const fillColor=hex(o.fillColor||o.color);page.drawEllipse({x:X((o.x1+o.x2)/2),y:Y((o.y1+o.y2)/2),xScale:Math.abs(o.x2-o.x1)*width/2,yScale:Math.abs(o.y2-o.y1)*height/2,borderColor:color,borderWidth:th,color:fillColor,opacity:o.fill?Math.max(0,Math.min(1,Number(o.fillOpacity??1))):0,borderOpacity:1});}`,
  'PDF export fill'
);
